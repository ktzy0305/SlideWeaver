...
const getSlideDataFromDom = () => {
    ...
    // Extract table data from table element
    const extractTableData = (table) => {
        const rows = [];

        // Get all rows from thead, tbody and tfooter
        ...
        trElements.forEach((tr) => {
            ...
            cellElements.forEach((cell) => {
                ...
                if (hasFormatting) {
                    ...
                    const isBold = cellComputed.fontWeight === 'bold' || parseInt(cellComputed.fontWeight) >= 600;
                    const baseRunOptions = {
                        ...
                        bold: isBold && !shouldSkipBold(cellComputed.fontFamily),
                        italic: cellComputed.fontStyle === 'italic',
                        underline: cellComputed.textDecoration.includes('underline')
                    };
                    cellText = parseInlineFormatting(cell, baseRunOptions, [], (x) => x);
                } else {
                    // Plain text
                ...
                // Text formatting
                const isBold = isHeader || cellComputed.fontWeight === 'bold' || parseInt(cellComputed.fontWeight) >= 600;
                if (isBold && !shouldSkipBold(cellComputed.fontFamily)) cellOptions.bold = true;
                if (cellComputed.fontStyle === 'italic') cellOptions.italic = true;
                if (cellComputed.textDecoration && cellComputed.textDecoration.includes('underline')) cellOptions.underline = true;

                cellOptions.fontSize = pxToPoints(cellComputed.fontSize);
                cellOptions.fontFace = cellComputed.fontFamily.split(',')[0].replace(/['"]/g, '').trim();
                cellOptions.color = rgbToHex(cellComputed.color);

                // Background color
                if (cellComputed.backgroundColor && cellComputed.backgroundColor !== 'rgba(0, 0, 0, 0)') {
                    cellOptions.fill = rgbToHex(cellComputed.backgroundColor);
                }

                // Alignment
                cellOptions.align = cellComputed.textAlign === 'start' ? 'left' : cellComputed.textAlign;

                const vAlign = cellComputed.verticalAlign;
                if (vAlign === 'middle') cellOptions.valign = 'middle';
                else if (vAlign === 'bottom') cellOptions.valign = 'bottom';
                else cellOptions.valign = 'top';

                // Padding (convert to points)
                const padding = pxToPoints(cellComputed.padding);
                if (padding > 0) cellOptions.margin = padding;

                // Borders
                const borderTop = parseFloat(cellComputed.borderTopWidth) || 0;
                const borderRight = parseFloat(cellComputed.borderRightWidth) || 0;
                const borderBottom = parseFloat(cellComputed.borderBottomWidth) || 0;
                const borderLeft = parseFloat(cellComputed.borderLeftWidth) || 0;

                // If there's any border, use the top border as representative
                if (borderTop > 0 || borderRight > 0 || borderBottom > 0 || borderLeft > 0) {
                    const borderWidth = Math.max(borderTop, borderRight, borderBottom, borderLeft);
                    const borderColor = cellComputed.borderTopColor || cellComputed.borderColor;
                    cellOptions.border = {
                        type: 'solid',
                        pt: borderWidth * PT_PER_PX,
    ...
    document.querySelectorAll('*').forEach((element) => {
        ...
        if (element.tagName === 'TABLE') {
            ...
            element.querySelectorAll('tr, td, th, thead, tbody').forEach(el => processed.add(el));
            processed.add(element);
            return;
        }
